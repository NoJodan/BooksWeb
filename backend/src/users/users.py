...
from utils.users import validate_username, validate_user, get_username, validate_user_by_id, get_user_id
...
def delete_user(user_id):
    ...
    profile_image = mongo.db.users.find_one({'_id': user_id}).get('profile_image')
...
def update_user(user_id):
    ...
    if user.get('profile_image'):
        previous_profile_image = mongo.db.users.find_one({'_id': user_id}).get('profile_image')
        if previous_profile_image != "default.jpg":
            delete_profile_image(previous_profile_image)
        set_user['profile_image'] = process_profile_image(user.get('profile_image'))
